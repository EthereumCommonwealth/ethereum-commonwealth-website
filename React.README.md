...
it('renders welcome message', () => {
  ...
  const welcome = <h2>Masthead to React</h2>;
...
Here's an example of using `react-testing-library` and `jest-dom` for testing that the `<App />` component renders "Masthead to React".
...
it('renders welcome message', () => {
  ...
  expect(getByText('Masthead to React')).toBeInTheDOM();
});
```
